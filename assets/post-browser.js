...
export function initializePostBrowser(document, window) {
  ...
  excludeButtons.forEach((button) => button.addEventListener("click", (event) => {
    ...
    ({ selected, excluded } = normalize(selected, excluded));
    // Only keyboard activation may move focus to another tag. On Restore the
    // action becomes CSS-hidden, so the generic fallback would otherwise focus
    // the first chip (often Culture) and expose its X without any hover.
